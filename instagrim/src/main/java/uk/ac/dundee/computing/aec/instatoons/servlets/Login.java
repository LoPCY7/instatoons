...
import uk.ac.dundee.computing.aec.instatoons.stores.RegErrors;
...
public class Login extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String avatar=null;
        int errorNumber=0;
        RegErrors logError= new RegErrors();
        ...
        if (isValid){
            ...
            System.out.println("Session in servlet "+session);
            response.sendRedirect("/Instatoons-PMessios/Profile");
            
        }else{
        	errorNumber=1;
        	logError.setLogError(errorNumber);
        	RequestDispatcher rd = request.getRequestDispatcher("/login.jsp");
            rd.forward(request, response);
        }
        logError.resetLogError(null);
        logError.setLogError(0);
